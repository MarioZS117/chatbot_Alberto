...
def obtener_orden_activa_por_usuario(usuario_id):
    """Devuelve una lista de órdenes activas para el usuario dado."""
    if usuario_id is None:
        return []
    sql = """
    SELECT o.id, COALESCE(p.nombre, '') AS platillo, o.cantidad, o.total, o.creado_en
    FROM ordenes o
    LEFT JOIN platillos p ON o.platillo_id = p.id
    WHERE o.usuario_id = %s AND o.estado = 'activa'
    ORDER BY o.creado_en DESC
    """
    with get_connection() as conn:
        with conn.cursor() as cur:
...
def obtener_cita_activa_por_usuario(usuario_id):
    """Devuelve una lista de citas activas para el usuario dado."""
    if usuario_id is None:
        return []
    sql = """
    SELECT id, asunto, fecha, creado_en
...
def cancelar_orden(orden_id):
    ...
    with get_connection() as conn:
        with conn.cursor() as cur:
            cur.execute("UPDATE ordenes SET estado = 'cancelada' WHERE id = %s", (orden_id,))
...
def get_orden_id_from_details(usuario_id, platillo_name, cantidad):
    """Obtiene el ID de una orden específica basada en sus detalles."""
    sql = """
    SELECT o.id 
    FROM ordenes o
    LEFT JOIN platillos p ON o.platillo_id = p.id
    WHERE o.usuario_id = %s AND p.nombre = %s AND o.cantidad = %s AND o.estado = 'activa'
    ORDER BY o.creado_en DESC
    LIMIT 1
    """
    with get_connection() as conn:
        with conn.cursor() as cur:
            cur.execute(sql, (usuario_id, platillo_name, cantidad))
            row = cur.fetchone()
            return row['id'] if row else None